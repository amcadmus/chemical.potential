...
int main (int argc, char * argv[])
{
  ...
  resdindex1.resize(nwat * 3 + nch4);
  atomindex1.resize(nwat * 3 + nch4);
  resdname1.resize(nwat * 3 + nch4);
  atomname1.resize(nwat * 3 + nch4);
  posi1.resize(nwat * 3 + nch4);
  velo1.resize(nwat * 3 + nch4);

  for (unsigned i = 0; i < nwat * 3; ++i){
  ...
  for (unsigned i = 0; i < nch4; ++i){
    resdindex1[nwat * 3 + i] = resdindex[nwat * 3 - 1] + i + 1;
    atomindex1[nwat * 3 + i] = atomindex[nwat * 3 - 1] + i + 1;
    resdname1[nwat * 3 + i] = "Meth";
    atomname1[nwat * 3 + i] = "CH4";
    posi1[nwat * 3 + i] = posi[nwat * 3 + i * 3];
    velo1[nwat * 3 + i] = velo[nwat * 3 + i * 3];
  }
