...
\author{Han Wang\inst{1,2}\fnmsep\thanks{\email{wang\_han@iapcm.ac.cn}}, Animesh Agarwal\inst{3}}
%
...
{It has been shown that, without the energy conservation, the force interpolation AdResS achieves
...
% cannot be simply computed from the equilibrium ensemble averages.
...
A typical example is the velocity auto-correlation\footnote{
  ...
  % of the spontaneous fluctuation of the molecular velocity, being more precisly,
...
\begin{figure}
  ...
  \caption{A schematic plot of an AdResS simulation. Inside the red circle is the atomistic region. Between the red and blue circles is the hybrid region. Outside the blue circle is the coarse-grained region.}
...
AdResS uses a scalar weighting function $w(\vect r),\ \vect r\in\Omega$ 
...
all interactions\footnote{The long-range electrostatic interactions
  are also treated by the cut-off method. {It is possible by using e.g.~the Reaction-Field method~\cite{onsager1936electric,tironi1995generalized}
  to calculate the electrostatic interactions.}
} being treated
by the cut-off method\footnote{{AdResS uses the same general cut-off method as in normal MD simulations~\cite{frenkel2001understanding}.}}, and the thickness of
...
To set up an AdResS simulation, one only needs to define the
...
both laws. The violation of these mechanical conservation laws has substantial influence on the thermodynamic
...
force exerted on one molecule is the sum of all pairwise forces:
\begin{align}
...
% {Remark, add the remark of inversed weighting function}
% \vskip .2cm
...
If the configuration of the atomistic region in AdResS 
is same as that of the corresponding subregion in the atomistic reference, then
AdResS is of good accuracy (see the comparison indicated by the red arrow in Fig.~\ref{fig:compare}).
...
{samples} the grand-canonical ensemble. Therefore, it is natural to investigate
if the atomistic region of AdResS also {samples} the grand-canonical ensemble
...
% {comments on the ``atomistic region'' of the reference system}
\begin{figure}
  \centering
...
$N_\AT$, $V_\AT$ and $T_\AT$ and those of the hybrid and coarse-grained regions
...
Hamiltonian, {and $\beta$ denotes the inverse temperature $1/(k_BT)$.}
...
\end{align}
where the thermodynamic force is a one-body force defined over space,
...
is updated only in the hybrid region. When the density in hybrid region is flat, the thermodynamic
...
In Ref.~\cite{wang2013grand}, the authors instead tested 
Weeks-Chandler-Andersen (WCA) potential~\cite{weeks1971role} as the
coarse-grained potential, which is a short-ranged and pure repulsive
interaction.  For an SPC/E water system, the cut-off radius of WCA potential
can be 2.4 times smaller than the cut-off radius used in the atomistic
...
that are {beyond equilibrium} ensemble averages.
...
% {the difficulty of Langevin thermostat: destroy dynamics. Test local thermostating~\cite{wang2014exploring}
% }
...
The rest of the system is coarse-grained by the WCA interaction.
The dynamical region  is
also centered at the $\alpha$-carbon, and is of radius 1.2~nm.
It has been proven that the conformational dynamics of alanine dipeptide is not sensitive to the size of
...
are treated using cut-off scheme. The cut-off used in both the cases is 1.0~nm.
...
those of the full atomistic reference.
{
  ...
  % (This can be easily seen by, e.g., considering a simplified 2-state reversible Markov model).
  % Moreover, the
...
auto-correlation functions of the characteristic functions that are not {purely} equilibrium ensemble averages.
...
Although these positive numerical evidences can neither answer the open questions
...
and suggest a direction for theoretical investigations.
...
than the atomistic interactions, one way to further boost the performance is
