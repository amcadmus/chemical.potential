...
\begin{figure}
  ...
  \caption{The oxygen velocity auto-correlation by atomistic
    ...
    Langevin thermostat, respectively.  The square and cross points
...
in sampling the equilibrium ensemble. Therefore, all quantities that
can be written down in terms of ensemble averages should have good
when using AdResS. However, not all quantities are in the form
of ensemble averages, for example, the velocity auto-correlation function
...
between the velocity at time $0$ and $t$.  In the context of molecular
dynamics simulation, these quantities does depends on how the dynamics
of the system is implemented, so they are called dynamical properties
of the system.  An illustrative example, presented in
Fig.~\ref{fig:vac}, is computing the oxygen velocity auto-correlation
in a TIP3P~\cite{jorgensen1983comparison} water system: If the system
is coupled to a Langevin thermostat, the result is quanlitatively
differed from that without any thermostat coupling, i.e.~Hamiltonian
...
is coupled,  is thus not suitable to reproduce the dynamical
...
at the desired  thermodynamic state. In this context, the dynamical quantities
should be measured only in the atomistic region. 


The local thermostat is validated by an AdResS simulation of pure TIP3P
water system.
The simulation is performed by the Gromacs~\cite{pronk2013gromacs} version 4.6.5 with AdResS support.
The system contains 5000 water molecules.  The dimension of the simulation region is $14.7\times3.2\times3.2~\textrm{nm}^3$,
in which the resolution changes only on $x$-direction.
The atomistic region is of size $1.5\times3.2\times3.2~\textrm{nm}^3$, and governed by Hamiltonian dynamics.
The rest of the system (hybrid + coarse-grained
regions) is coupled to Langevin thermostat of time-scale 
$\tau_T = 0.1$~ps.
The WCA potential
% with parameter $\sigma = 0.3~$nm and $\epsilon = 0.65$~kJ/mol
is adopted as generic coarse-grained water,
and the weighting function of form Eq.~\eqref{eqn:new-w} is used to couple different resolutions with
two hybrid regions of width $d_{\HY} = 2.85$~nm.
...
functions computed by fully equilibriated of length 1~ns, with the
...
atomistic simulation, and show almost perfect consistency.
This indicates that the AdResS with the local thermostat
is a promising method to investigate the dynamics quantities
...
a spherical atomistic region of radius  $0.5$~nm centered at the $\alpha$-carbon and a hybrid shell-shaped region of width 2.0~nm.
The rest of the system is treated coarse-grainly by the WCA potential.
The dynamical region (no thermostating region) is
...
% \begin{align}\label{eqn:timescale}
...
% The timescale of the slow part of the relaxation is govenerned by the leanding eigenvalues through Eq.~\eqref{eqn:timescale},
