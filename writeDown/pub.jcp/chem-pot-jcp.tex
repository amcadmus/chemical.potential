...
formula \bluec{(being this latter, in GC-AdResS, generated by the non conservative effect of the force interpolation).}
...
It follows that (for example) for component $\typea$:
\begin{equation}
\mu^{A}_{CG}-\mu^{A}_{AT}=\int _{\Delta}{\bf F}^{A}_{th}({\bf r})d{\bf r}+\int_{\Delta} \langle \vect F^{\typea\typea}_\res(\vect r)\rangle d{\bf r}+\int_{\Delta} \langle \vect F^{\typea\typeb}_\res(\vect r)\rangle d{\bf r}
\label{fineq}
\end{equation}
