...
\bluec{
  ...
  \item define COM, change all ``center of mass'' to COM
  \item change the title of sec.III
  \item do not be too mean, we cite BOTH of their PRL H-Adress paper...
  \item some times, it is called GC-AdResS, sometimes original AdResS, sometimes standard AdResS, sometimes force-interpolation AdResS....
  \item rewrite eq. 4, see if it is OK for you
  \end{itemize}
}
...
\section{Analytic Deriviation of the $w_{\ext}$}
...
It is straightforward to show that \recheck{HAN, here it must be shown} \bluec{No, who cannot derive it should be sent back to the graduate course of thermodynamics.} 
...
\begin{align}\label{eqn:w-rep}
  ...
  = \int_\HY d \vect r\,\nabla_{\vect r}\omega(\vect r)\,
  \big\langle \omega(\vect r')\, [U^\AT(\vect r-\vect r') - U^\CG(\vect r-\vect r')]\big\rangle_{\vect r'; \vect r}
\end{align}
\recheck{The average is taken over all possible positions of the second molecule, provided the position of the first molecule
