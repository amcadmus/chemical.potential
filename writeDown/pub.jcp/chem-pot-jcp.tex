...
\recheck{Moreover, in recent work AdResS has been merged with the MARTINI force field \cite{matej-sw1,matej-sw2}. In this context, the possibility of having a quantity like the chemical potential under \bluec{what is ``explicit control''} can be used as a further argument for the validity of the method in applications to large systems of biological interest.}
...
\section{Analytic Deriviation of the $w_{\ext}$}\label{sec:tmp4}
{In this section we derive analytically the equivalence:
  $\omega_{\ext}=\int_{\Delta}\nabla_{\vect r} w(\vect r) \langle w(U^{\AT}-U^{\CG})\rangle_{\vect r} d{\vect r}$ and define its limitations.
...
%designed. Therefore, we will take the advantage of both and
...
correlated (short-ranged must be intended as a range comparable to the size of the filter). The thermodynamic force for GC-AdResS ($\vect F_\thf$) \recheck{and for the auxiliary Hamiltonian AdResS ($\vect F^\hadress_\thf$),}
...
Instead when we consider the auxiliary Hamiltonian approach, the third part of the R.H.S.~of Eq.~\eqref{eqn:interpol-f} is not
...
Therefore, the change of the free energy of the system is \recheck{approximately}:
\begin{align}\nonumber
  \Delta A \approx&\,
...
\recheck{Since the system is in the thermodynamic limit, and the interactions in the system are short-ranged,
  the total free energy has the above splitting form.}
% \recheck{HAN: why is it justified to split the free energy in free energies of subsystems and why the free energy of the HYB region is only the work and not also its kinetic component. I understand that since the molecule do not change resolution, their kinetic part stays the same and thus the difference is zero. However this formula needs either a reference or few more explanations}.
$N_1$ and $N_3$ are the numbers of molecules in the AT and CG region, and
$V_1$ and $V_3$ are the volumes in the AT and CG region, respectively. $T$ is the temperature
of the system.
The last term comes from the work done on the piston. It is composed by
two parts, the first is the work done by the thermodynamic force, and the
second is due to 
the force of changing representation, which does not vanish due to the breaking of the Newton's
action-reaction law. The first and second part of Eq.~\eqref{eqn:interpol-f}
do not contribute, because they obey the Newton's action-reaction law, so
their total work is zero as long as the transition region move infinitesimally in one direction.
The notation $\langle\cdot\rangle$ in Eq.~\eqref{eqn:apptmp3} denotes the ensemble averge.
It is straightforward to show that
% \recheck{HAN, here it must be shown} \bluec{No, who cannot derive it should be sent back to the graduate course of thermodynamics.} 
...
\end{align}
\bluec{The average is taken over all
  possible positions of the second molecule (i.e.~$\vect r'$),
  provided the position of the first molecule (i.e.~$\vect r$)
  in the pairwise interaction.
In case of molecules that contain
more than one atom, the averge is also taken over all possible
conformations in the atomistic resolution.}
% By using the same argument as the Section III.C of Ref.~\cite{prx} \recheck{Han, which ones?}, we can
% show that
% \bluec{here since we have the auxiliary Hamiltonian, then the system can be treated by the conclusions
%   that can be found in standard thermodynamic text books. If the referee does not understand, he should
% be sent back to the graduate cource. }
In the thermodynamic limit, the equilibrium volume of the AT region maxiimize the
free energy, i.e.~$\Delta A / \Delta V
= 0$, which yields
\begin{align}\label{eqn:peq-h}
  p_\CG - p_\AT =  \rho_0 (\omega_\thf^\hadress - \omega_\res).
\end{align}
Comparing with the result of the standard GC-AdResS~Eq.~\eqref{eqn:peq},
we have: 
...
\end{align}
Since we have an auxiliary Hamiltonian, we do not have the term $w_\ext$ in the above
fomula.
...
interpolation approach is used.  It is obvious that the phase spatial probability distribution correponding to the system treated with the potential interpolation is consistent with the force
interpolation at least up to the 1st order as defined in Eq.\ref{order}.  It is also
possbile to systematically obtain equivalence in the ensemble average operation  at higher orders of accuracy of the probability distribution, as, for example, it is done for the radial distribution function in Ref.~\cite{jctchan}.
...
\end{align}
In the next section we apply the procedure to the calculation of the excess chemical potential of several liquids and mixtures.
...
The chemical potential of $k$-th liquid's component in a mixture is calculated as:
\begin{equation}
  \mu^{\exc,k}_{\AT}=
  \mu^{\exc,k}_{\CG}-
...
\recheck{The ensemble evarage is taken over the position of the second molecule, provided that the first moelcule is of species $k$, and located at postion $\vect r$}.
% $\nabla_{x_{i}} w(x)$ is the gradient along $x$ (in a rectangular box) applied to the switching function $w(x)$ in the COM of molecules of component $i$. 
...
\recheck{In principle when the auxiliary Hamiltonian approach is used,
  ...
  use of a thermostat. \bluec{I do not understand the following sentence... cannot revise it
    plz make it short and clear. thanks..}
  \bluec{In this case the thermodynamic force of the
  auxiliary Hamiltonian does not have any effect of the thermostat and
  thus its difference from the thermodynamic force of GC-AdResS is
  exactly the work that the thermostat does in GC-AdResS in order to
  enforce the falt density profile and thus induced the correct flux
  of particles which are changing resolution.} The question is whether
  ...
  procedure is straightforward. Instaed, for systems with
...
\section{Extending the chemical potential expression to multi-component system}


In this section we explain how to extend the chemical potential
expresstion Eq.~\eqref{eqn:tmp24} for pure system to multi-component
system, i.e.~\eqref{eqn:tmp25}.
For simplicity and without lost of generality, we assume that the system
is composed by two components, and the number of molecules are
$N^\typea$ and $N^\typeb$, respectively. The superscript ``$\typea$'' and ``$\typeb$'' throughout this section 
denotes the index of the components.  We further denote the number of 
molecule $\typea$ in the atomistic, transition, and coarse-grained regions by
$N^\typea_1$, $N^\typea_2$ and $N^\typea_3$, respectively. While those for type $\typeb$ molecules
are $N^\typeb_1$, $N^\typeb_2$ and $N^\typeb_3$, respectively.  By
assuming the transition region is infinitely small comparing with the
...
\end{align}
By calculating the thermodynamic forces for each of the components, which
are denoted by $\vect F^\typea_\thf$ and $\vect F^\typeb_\thf$,
...
\end{align}
These can be achieved by the iterative thermodynamic force scheme for each
component in the mixture system.\\

Similar to Eq.~\eqref{eqn:peq}, we have for the force interpolation AdResS:
\begin{align}\label{eqn:peq-d-ab}
  p_\CG - p_\AT=\rho_0^\typea \omega^\typea_\thf + \rho_0^\typeb \omega^\typeb_\thf
\end{align}
Following the same argument in Sec.~\ref{sec:tmp4}, for $\typea$--$\typeb$
mixture we have
...
\end{align}
where the work of change representation for molecule $\typea$ is defined by
...
\end{align}
Here work of changing representation is splitted based on the
type of interaction, e.g.~$\omega_\res^{\typea\typeb}$ is due to the
cross interaction between molecule $\typea$ and $\typeb$.
...
denotes the atomistic interaction between molecule $\typea$ and $\typeb$.
Notation $\langle\cdot\rangle_{\vect r',\typea; \vect r,\typeb}$ denotes the
ensemble average taken with respect to position $\vect r'$ being molecule
$\typea$, provided a molecule $\typeb$ taking the postion $\vect r$.
If the molecules contain more than one atom, then the average is also taken
over all possible conformations.
Therefore, the physical meaning of the
force $\langle \vect F^{\typea\typeb}_\res(\vect r)\rangle$
is that providing a molecule of type $\typea$ at $\vect r$,
the averaged force between this atom and all molecule type $\typeb$
in the system.
...
We denotes the work done by the transition region on the two types
...
\begin{align}\label{eqn:tmpc7}
  \mu^\typea_\AT(N^\typea_1, N^\typeb_1, V_1, T) &= \mu^\typea_\CG(N^\typea_1, N^\typeb_1, V_1, T) - \omega^\typea_0\\\label{eqn:tmpc8}
  \mu^\typeb_\AT(N^\typea_1, N^\typeb_1, V_1, T) &= \mu^\typeb_\CG(N^\typea_1, N^\typeb_1, V_1, T) - \omega^\typeb_0
\end{align}
Now, $N^\typea_1$ and $N^\typeb_1$ has the meaning of the averaged number of type $\typea$ and
type $\typeb$ molecules in the atomistic region, respectively.  Under the thermodynamic
...
before, because the exact meaning is clear from the context.
The meaning of the chemical potential, e.g.~$\mu_\AT^\typea$, is clear:
it is the free energy increament of inserting one type $\typea$ molecule
into the infinitely large $\typea$--$\typeb$ mixture.\\

Similar to the pure system, from Eq.~\eqref{eqn:tmpc7} and \eqref{eqn:tmpc8},
we write down  for the force interpolation AdResS:
...
\end{align}
The meaning of $ \omega^\typea_\thf$ and $ \omega^\typeb_\thf$ are clear:
...
The energy dissipation can be further splitted as
...
$\typea$ produced by only non-conservative interactions between
molecule type $\typea$ and type $\typea$.
While $\omega^{\typea\typeb}_\ext$ is
the energy dissipation of a molecule $\typea$
due to the non-conservative cross interaction between
type $\typea$ and $\typeb$.  The defintions are similar for $
\omega^{\typeb\typea}_\ext$ and $ \omega^{\typeb\typeb}_\ext$.
It should be noticed that, we do not have
$\omega^{\typeb\typea}_\ext = \omega^{\typea\typeb}_\ext$ in gneneral.
The same argument is applied to the auxiliary Hamiltonian approach, and
yields
...
$\omega_\ext^{\typea\typea} = \omega_\res^{\typea\typea} $  and
...
From the physical meaning of $\omega_\ext^{\typea\typeb}$,
$\omega_\res^{\typea\typeb}$, $\omega_\ext^{\typeb\typea}$ and
$\omega_\res^{\typeb\typea}$, it is the most natural way to further
identify $\omega_\ext^{\typea\typeb} = \omega_\res^{\typea\typeb}$
and $\omega_\ext^{\typeb\typea}=\omega_\res^{\typeb\typea}$.
In summary, we prove the extension of chemical potential expression
for multi-component system, i.e.~Eq.~\eqref{eqn:tmp25}.
